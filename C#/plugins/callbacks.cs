...
using plugins.helpers = Helpers;
using pyrogram.types = CallbackQuery;
using plugins.settings = user_settings, default_emojis_settings;
...
async def change_default_emojis_cb_func(bot: Stark, query: CallbackQuery)
{
    user_id = query.from_user.id;
    await query.answer();
    emojis_msg = await bot.ask(
        user_id, "Please send me the emojis", filters=filters.text & filters.incoming
    );
...
async def change_bool(key, query)
{
    ...
    {
        await query.answer(
            "An error occurred. Try a second time, If it persists ask in support chat"
        );
        return;
    }
    await query.edit_message_text(text, reply_markup=markup);
